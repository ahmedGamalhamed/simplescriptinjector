{
  (async () => {
    ...
    if (!text) {
      if (div) {
        ...
        display == "block" ? (div.style.display = "none") : (div.style.display = "block");
      }
      return;
    }
  ...
  async function fetchAnswer(selectionText) {
    try {
      console.log("fetch");
      let openAiKey = "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";
      let apiRequestBody = {
        model: "gpt-3.5-turbo-0301",
        messages: [
          { role: "system", content: "understand and expand upon the upcoming text: " },
          {
            role: "user",
